...
public class SongRepository {
    ...
    public void getArtistSong( String song ) {
        ...
        entityManager.getTransaction().begin();
        TypedQuery<ICAI.entity.Songs> query = entityManager.createQuery("select s.artistSet from Songs s where s.name =?1", ICAI.entity.Songs.class);

        query.setParameter(1, song );

        List<ICAI.entity.Songs> songs = query.getResultList();
        System.out.println(songs);
        entityManager.getTransaction().commit();
        entityManager.close();
